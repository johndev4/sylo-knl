# Workspace Member Management - Architecture & Implementation Details
...
│  • removeMultipleWorkspaceMembers(workspaceId, userIds[])           │
└─────────────────────────────────────────────────────────────────────┘
                              ↕ (HTTP API calls)
┌─────────────────────────────────────────────────────────────────────┐
│                        API Routes Layer                              │
...
│         ├─ Permission check (member of workspace)                   │
...
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │ workspace_id (uuid)  │ user_id (uuid)  │ role (enum)         │  │
│  │ created_at (timestamp)                                        │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                                                                       │
│  RLS Policies:                                                       │
│  ├─ SELECT: User is member of workspace                             │
│  ├─ INSERT: Owner/Admin adding OR user adding self                  │
│  ├─ UPDATE: Owner/Admin only                                        │
│  └─ DELETE: Owner/Admin only                                        │
│                                                                       │
│  Triggers:                                                           │
│  └─ MAX 11 MEMBERS per workspace                                    │
└─────────────────────────────────────────────────────────────────────┘
```
...
2. AddMemberForm validates email format (Zod)
   ↓
3. Form calls: addWorkspaceMember(workspaceId, email, role)
   ↓
...
5. API Handler:
   ...
   b) Check if user is member of workspace → 403 if not
   ...
   h) INSERT into workspace_members
   i) Return member object with user details
   ↓
6. MemberTable re-fetches members via fetchWorkspaceMembers()
   ↓
...
1. User selects new role from dropdown
   ↓
2. Calls: updateWorkspaceMemberRole(workspaceId, userId, newRole)
   ↓
3. PATCH /api/workspaces/[id]/members/[userId]
   {
     "role": "ADMIN"
   }
   ↓
...
3. User confirms deletion
   ↓
4. Calls: removeWorkspaceMember(workspaceId, userId)
   ↓
5. DELETE /api/workspaces/[id]/members/[userId]
   ↓
6. API Handler:
   a) Get current user from Supabase auth
   b) Check if user is OWNER or ADMIN → 403 if not
   ...
   e) DELETE from workspace_members
   f) Return success
   ↓
...
```
WorkspaceSettingsPage (page.tsx)
...
| File | Purpose |
|------|---------|
| `workspace.schema.ts` | Zod validation schemas |
| `workspace.service.ts` | Business logic services |
