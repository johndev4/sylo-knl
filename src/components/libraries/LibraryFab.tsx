...
export function LibraryFab({ onUpload, onNewChat }: LibraryFabProps) {
  ...
  return (
    ...
    >
      <div className="rounded-full bg-slate-950/95 px-4 py-3 text-sm font-medium tracking-[0.02em] text-slate-100 text-white shadow-2xl ring-1 ring-slate-900/20 backdrop-blur-xl">
        Quick actions
      </div>
      {onUpload && (
        <Button
      ...
      <div className="rounded-full bg-white/90 px-3 py-2 text-xs text-slate-600 shadow-sm ring-1 ring-slate-900/10 backdrop-blur">
        <Sparkles className="mr-1 inline h-3 w-3 text-amber-500" /> Instant
        library actions
      </div>
    </motion.div>
  );
